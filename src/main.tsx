...
const LoginPage = ({ onLogin }: { onLogin: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        data.employee.branch_id !== undefined &&
        data.employee.branch_id !== null
      ) {
        const id = Number(data.employee.branch_id);
        setBranchId(id);
        localStorage.setItem("branchId", String(id));
      }
      // Update AppContext with user role immediately
      if (data.employee.role) {
        const role = String(data.employee.role);
        setUserRole(role);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 px-4">
      ...
      >
        ...
        <div className="mb-4">
          <label
            htmlFor="username"
            className="block mb-1.5 text-xs font-medium text-gray-600 dark:text-gray-300"
          >
            Username
          </label>
